...
class JetInvokeFunctionReference extends JetPsiReference implements MultiRangeReference {
    ...
    protected Collection<? extends DeclarationDescriptor> getTargetDescriptors(@NotNull BindingContext context) {
        ...
        ResolvedCall<? extends CallableDescriptor> invokeFunction = context.get(RESOLVED_CALL, calleeExpression);

        if (invokeFunction != null && invokeFunction instanceof VariableAsFunctionResolvedCall) {
            return Collections.singleton(((VariableAsFunctionResolvedCall) invokeFunction).getResultingDescriptor());
        }
        return null;
    }
